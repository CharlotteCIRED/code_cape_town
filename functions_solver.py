...
def compute_housing_supply_formal(R, options, housing_limit, param, agricultural_rent, interest_rate, fraction_capital_destroyed, minimum_housing_supply, construction_param, housing_in):
    ''' Calculates the housing construction as a function of rents '''

    if options["adjust_housing_supply"] == 1:
    
        housing_supply = 1000000 * (construction_param ** (1/param["coeff_a"])) * ((param["coeff_b"] / (interest_rate + param["depreciation_rate"] + fraction_capital_destroyed.structure)) ** (param["coeff_b"]/param["coeff_a"])) * ((R) ** (param["coeff_b"]/param["coeff_a"]))
...
def compute_housing_supply_backyard(R, param, income_net_of_commuting_costs, fraction_capital_destroyed):
    """ Calculates the backyard available for construction as a function of rents """

    housing_supply = (param["alpha"] * (param["RDP_size"] + param["backyard_size"] - param["q0"]) / (param["backyard_size"])) - (param["beta"] * (income_net_of_commuting_costs[0,:] - (fraction_capital_destroyed.structure * param["subsidized_structure_value"])) / (param["backyard_size"] * R))
...
def compute_dwelling_size_formal(utility, amenities, param, income_net_of_commuting_costs, fraction_capital_destroyed):
    ...
    left_side = (utility[:, None] / amenities[None, :]) * ((1 + (param["fraction_z_dwellings"] * fraction_capital_destroyed.contents[None, :])) ** (param["alpha"])) / ((param["alpha"] * income_temp) ** param["alpha"])
    ...
    print(np.nanmedian(np.abs(dwelling_size - approx / approx)) * 100, '%')

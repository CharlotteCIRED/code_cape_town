...
df_flood = np.squeeze(pd.read_excel("C:/Users/Charlotte Liotta/Desktop/cape_town/2. Data/FATHOM/" + 'FD_20yr' + ".xlsx"))
WBUS2_20yr = pd.read_excel("C:/Users/Charlotte Liotta/Desktop/cape_town/2. Data/Flood plains - from Claus/WBUS2_20yr.xlsx")
df_flood.prop_flood_prone = np.fmax(df_flood.prop_flood_prone, WBUS2_20yr.prop_flood_prone)

    
# %% Run initial state for several values of amenities

#List of parameters
list_amenity_backyard = np.arange(0.65, 0.90, 0.01)
list_amenity_settlement = np.arange(0.65, 0.90, 0.01)
housing_type_total = np.zeros((3, 6, len(list_amenity_backyard) * len(list_amenity_settlement)))
...
for i in range(0, len(list_amenity_backyard)):
    for j in range(0, len(list_amenity_settlement)):
        ...
        housing_type_total[2, 0:4 , index] = sum_housing_types(initial_state_households_housing_types)
        housing_type_total[2, 4 , index] = np.nansum(df_flood.prop_flood_prone * initial_state_households_housing_types[2, :])
        housing_type_total[2, 5 , index] = np.nansum(df_flood.prop_flood_prone * initial_state_households_housing_types[1, :])
...
distance_share_score = distance_share[1,:] + distance_share[2,:] + (max(housing_type_total[2, 5, :]) - housing_type_total[2, 5, :])
